...
%     lower_firing_strength = prod(lower_membership_values,2) + 1e-4;
%     upper_firing_strength = min((prod(upper_membership_values,2) + 1e-4), 1);
%     lower_firing_strength = relu(prod(lower_membership_values,2) - 1e-14);
%     upper_firing_strength = relu(prod(upper_membership_values,2) - 1e-14);

    lower_firing_strength = prod(lower_membership_values,2);
%     lower_firing_strength = lower_firing_strength./(max(lower_firing_strength));
    upper_firing_strength = prod(upper_membership_values,2);

%     lower_firing_strength = (lower_firing_strength.^4) ./ sum(lower_firing_strength.^4);
