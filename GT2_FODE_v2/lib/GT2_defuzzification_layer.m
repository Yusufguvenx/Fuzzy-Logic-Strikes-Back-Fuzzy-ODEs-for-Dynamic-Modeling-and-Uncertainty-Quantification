...
%         c = permute(c, [1, 3, 2]);
        ...
        delta_f = permute(delta_f, [1 2 4 3]); %added

        
        pay2 = permute(c, [3, 1, 2]).*delta_f;
        pay2 = pagemtimes(pay2, u);
        pay2 = permute(pay2,[3,2,1,4]);

        lower_firing_strength_to_be_used = permute(lower_firing_strength, [1 4 3 2]);%added

        pay1 = sum(c .* lower_firing_strength_to_be_used,1);
        pay1 = permute(pay1, [2, 1, 3 4]); %added

        pay = pay1 + pay2;
        pay  = permute(pay, [4 2 3 1]);%added
...
elseif output_type == "IVL"
    ...
    elseif type_reduction_method == "KM"
        ...
        linear_lower = reshape(linear_lower, [size(lower_firing_strength, 1), number_outputs, size(lower_firing_strength, 3)]);
        linear_upper = reshape(linear_upper, [size(upper_firing_strength, 1), number_outputs, size(upper_firing_strength, 3)]);
        ...
        pay2_lower = permute(linear_lower, [3, 1, 2]).*delta_f;
        pay2_lower = pagemtimes(pay2_lower, u);
        pay2_lower = permute(pay2_lower,[3,2,1]);

        pay1_lower = sum(linear_lower .* lower_firing_strength,1);
        pay1_lower = permute(pay1_lower, [2, 1, 3]);

        pay_lower = pay1_lower + pay2_lower;

        pay2_upper = permute(linear_upper, [3, 1, 2]).*delta_f;
        pay2_upper = pagemtimes(pay2_upper, u);
        pay2_upper = permute(pay2_upper,[3,2,1]);


        pay1_upper = sum(linear_upper .* lower_firing_strength,1);
        pay1_upper = permute(pay1_upper, [2, 1, 3]);
