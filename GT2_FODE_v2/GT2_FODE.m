...
% In this example, we implement GT2-Fuzzy ODE. During calculation of the
% loss function, we used the derlivative-like quantity of GT2-Fuzzy ODE.
...
alpha_rev = (permute(alpha, [1, 4, 2, 3])) / sum(alpha);
...
[Learnable_parameters, timesteps] = initialize_Glorot_GT2(Train.inputs, Train.outputs, input_type,output_membership_type, number_mf, t, neuralOdeTimesteps);
...
for epoch = 1: number_of_epoch
    ...
    for iter = 1:number_of_iter_per_epoch
        ...
        [loss, gradients, yPred_train_lower, yPred_train_upper, yPred_train] = dlfeval(@GT2_fismodelLoss_dlode, timesteps, mini_batch_inputs ,...
            number_inputs, u_mini_batch, targets,number_outputs, number_mf, mbs, Learnable_parameters, output_membership_type,...
            input_membership_type,input_type,type_reduction_method,u, alpha, delta, alpha_rev, neuralOdeTimesteps);
    ...
    if(epoch==1 || mod(epoch, plotFrequency) == 0)
        ...
        for ct = 1:ahead
            u_mini_batch = Ux(:, :, ct);
            [x_l, x_u, x_m] = evaluatemodel(x, u_mini_batch, Learnable_parameters,number_mf, number_inputs,number_outputs, mbs, output_membership_type, input_membership_type, input_type, type_reduction_method, u, alpha, delta, alpha_rev);
            X_mean(:, ct) = x_m';
            X_lower(:, ct) = x_l';
            X_upper(:, ct) = x_u';
            x = x_m;
