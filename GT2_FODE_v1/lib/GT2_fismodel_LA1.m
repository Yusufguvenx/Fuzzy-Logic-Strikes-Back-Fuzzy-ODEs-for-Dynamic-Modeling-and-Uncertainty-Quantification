function [output_lower, output_upper, output_mean] = GT2_fismodel_LA1(x, number_mf, number_inputs, number_outputs, mbs, learnable_parameters, output_type, input_mf_type, input_type, type_reduction_method, u, alpha, delta)
...
for i=1:numel(alpha)
    ...
    [cut(i).firestrength_lower, cut(i).firestrength_upper] = T2_firing_strength_calculation_layer(cut(i).smf_fuzzified_lower, cut(i).smf_fuzzified_upper, "min");
...
output_lower = cut(1).output_lower;
output_upper = cut(1).output_upper;
